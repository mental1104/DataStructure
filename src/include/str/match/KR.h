...
inline int krDigit(const String& S, size_t i) { return S[i] - '0'; }

using HashCode = std::int64_t;

inline HashCode prepareDm(size_t m) { // R^(m - 1) % M
    HashCode Dm = 1;
    for (size_t i = 1; i < m; i++) Dm = (kKRBase * Dm) % kKRMod;
    return Dm;
}

inline bool check1by1(const String& P, const String& T, size_t i) { // confirm exact match
    for (size_t m = P.size(), j = 0; j < m; j++, i++)
        if (P[j] != T[i]) return false;
    return true;
}

inline void updateHash(HashCode& hashT, const String& T, size_t m, size_t k, HashCode Dm) {
...
inline int matchKR(const String& P, const String& T, MatchObserver* obs = nullptr) { // Karp-Rabin
    ...
    for (size_t k = 0;;) {
        observer.onKRProgress(T, P, k, hashP, hashT);
        observer.onPause();
        if (hashT == hashP && check1by1(P, T, k)) return (int)k;
        if (++k > n - m) return (int)k;
