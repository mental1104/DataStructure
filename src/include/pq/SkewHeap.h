...
template <typename T>
class SkewHeap : public PQ<T>, public BinTree<T> { //基于二叉树，以斜堆形式实现的PQ
...
public:
    ...
    void merge(SkewHeap<T>& right);
    void insert(T); //按照比较器确定的优先级次序插入元素
    T getMax(); //取出优先级最高的元素
    T delMax(); //删除优先级最高的元素
}; //SkewHeap

template <typename T> //合并以a和b为根节点的两个斜堆
BinNode<T>* SkewHeap<T>::merge(BinNode<T>* a, BinNode<T>* b) {
    if ( !a ) return b;
    if ( !b ) return a;
    if ( a->data < b->data )
...
template<typename T>
void SkewHeap<T>::merge(SkewHeap<T>& right){
