...
template<typename T> 
Rank PQ_ComplHeap<T>::Bigger(Rank i, Rank j){
...
template<typename T> 
Rank PQ_ComplHeap<T>::ProperParent(Rank n, Rank i){/*父子（至多）三者中的大者*/
    if(RChildValid(n, i))
...
template <typename T> 
void PQ_ComplHeap<T>::heapify(const Rank n ) { //Floyd建堆算法，O(n)时间
